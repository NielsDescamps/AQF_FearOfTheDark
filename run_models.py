...
def plot_gaussian_pdf():
    ...
    if jump_distribution == "Exponential":
        ...
        jump_params[1] = np.array([[20, 10], [20, 20]])
        ...
        plt.plot(-k_joint, pdf_joint, label='Medium cross excitation', linestyle='-.', linewidth=1.1)
        plt.plot(-k_joint, pdf_joint_low, label='No cross excitation', linestyle= ':', linewidth=1.1)
        plt.plot(-k_joint, pdf_joint_high, label='High cross excitation', linestyle='--', linewidth=1.1)
...
def plot_exponential_pdf():
    ...
    jump_params[1] = np.array([[-50, 10], [10, -50]])
    PHI_joint = [joint_characteristic_function(val, t, T, heston_params['kappa'], heston_params['eta'],
                                                   heston_params['theta'],
                                                   heston_params['rho'], heston_params['v0'], heston_config['r'],
                                                   heston_config['q'],
    ...
    plt.figure()
    plt.plot(-k_heston, pdf_heston, label='Heston Model')
    plt.plot(-k_joint, pdf_joint, label='Medium cross excitation', linewidth=1.1, linestyle='-.')
    plt.plot(-k_joint, pdf_joint_low, label='No cross excitation', linewidth=1.1, linestyle=':')
    plt.plot(-k_joint, pdf_joint_high, label='High cross excitation', linewidth=1.1, linestyle='--')
    plt.xlabel('z [log-returns]')
    plt.ylabel('probability density')
...
def plot_heston_pdf():
    ...
    PHI_heston_high = [heston_characteristic(kappa, eta,  2, rho, sigma0, r, q, S0, T, val) for val in u]
...
def plot_XNG():
    ...
    plt.title('16.01.2010')
    plt.legend()
    plt.xlim(450, 610)
    plt.savefig("./generated_plots/XNG_call_prices.png")
    plt.show()
    ...
    plt.plot(df_calc["moneyness"], df_calc["Jump ImpliedVol G"], 'v', label="Exponential jump model implied vol", linewidth=1.1)
    plt.plot(df_calc["moneyness"], df_calc["Jump ImpliedVol E"], 'g2', label="Gaussian jump model implied vol", linewidth=1.1)
    plt.xlabel('Moneyness K/S0')
    plt.ylabel('Implied volatility')
    plt.title('16.01.2010')
    plt.legend()
    plt.xlim(0, 10)
    plt.savefig("./generated_plots/XNG_impliedVol.png")
    plt.show()
